import { getCacheKey, getPrices, parsePriceRecords } from './prices'
...
describe('getPrices', () => {
  const successResponse = {
    ...
    json: async () => ({
      data: [
        { date: '2026-06-11', close: 100 },
        { date: '2026-06-12', close: 110 },
      ],
    }),
  }
  ...
  test('retries failed symbol on next call instead of treating null as cached', async () => {
    const mockFetch = jest.spyOn(global, 'fetch')
      .mockRejectedValueOnce(new Error('network error'))
      .mockResolvedValueOnce(successResponse)

    const first = await getPrices(['2330'])
    expect(first['2330']).toBeNull()

    const second = await getPrices(['2330'])
    expect(second['2330']).not.toBeNull()
