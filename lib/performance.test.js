...
describe('computeWinRate', () => {
  ...
  test('net profitable sell → counted as win', () => {
    // costPrice = 101/share; sell 5 shares: netPnl = (600-5) - 101*5 = 595-505 = +90
    const trades = [
      t({ date: '2026-01-01', shares: 10, amount: 1000, fee: 10 }),
  ...
  test('net loss sell → counted as loss', () => {
    // costPrice = 101/share; sell 5 shares: netPnl = (400-5) - 101*5 = 395-505 = -110
    const trades = [
      t({ date: '2026-01-01', shares: 10, amount: 1000, fee: 10 }),
  ...
  test('two sells, one win one loss → 50%', () => {
    // costPrice = 101/share
    // sell1: netPnl = (360-3) - 101*3 = 357-303 = +54 → win
    // sell2: netPnl = (240-3) - 101*3 = 237-303 = -66 → loss
    const trades = [
      t({ date: '2026-01-01', shares: 10, amount: 1000, fee: 10 }),
      t({ date: '2026-02-01', type: '賣出', shares: 3, price: 120, amount: 360, fee: 3 }),
      t({ date: '2026-03-01', type: '賣出', shares: 3, price: 80,  amount: 240, fee: 3 }),
    ]
    expect(computeWinRate(trades)).toEqual({ wins: 1, total: 2, rate: 50.0 })
  })

  test('sell price above cost but net loss after fee → counted as loss', () => {
    // costPrice = 101/share; sell 5 shares at 102 but high fee (60)
    // netPnl = (510-60) - 101*5 = 450-505 = -55 → loss despite price > costPrice
    const trades = [
      t({ date: '2026-01-01', shares: 10, amount: 1000, fee: 10 }),
      t({ date: '2026-02-01', type: '賣出', shares: 5, price: 102, amount: 510, fee: 60 }),
    ]
    expect(computeWinRate(trades)).toEqual({ wins: 0, total: 1, rate: 0.0 })
  })
