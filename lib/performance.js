...
// Uses date-level granularity only. Multiple trades for the same symbol on
// the same calendar day may yield imprecise results since intra-day order
// cannot be determined without timestamps.
...
export function computeWinRate(trades) {
  ...
  for (const sell of sells) {
    ...
    if (!pos) continue
    matched++
    const netPnl = (sell.amount - sell.fee) - pos.costPrice * sell.shares
    if (netPnl > 0) wins++
  }
