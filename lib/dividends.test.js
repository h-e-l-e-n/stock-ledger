...
describe('computeReceivedDividends', () => {
  ...
  test('excludes dividend when shares were bought on the exDate itself', () => {
    const result = computeReceivedDividends(
      [div({ exDate: '2024-06-15' })],
      [trade({ date: '2024-06-15', shares: 1000, amount: 1000000, fee: 0 })]
    )
    expect(result).toHaveLength(0)
  })
})
...
describe('parseDividendRecords', () => {
  ...
  test('emits two records when cash and stock ex-dates differ', () => {
    const raw = [
      {
        CashExDividendTradingDate: '2024-06-15',
        StockExDividendTradingDate: '2024-09-20',
        CashEarningsDistribution: 3.5,
    ...
    expect(result[0]).toEqual({ symbol: '2330', exDate: '2024-06-15', cashDividend: 3.5, stockDividend: 0 })
    expect(result[1]).toEqual({ symbol: '2330', exDate: '2024-09-20', cashDividend: 0, stockDividend: 1.0 })
  })

  test('emits one record when cash and stock ex-dates are the same', () => {
    const raw = [
      {
        CashExDividendTradingDate: '2024-06-15',
        StockExDividendTradingDate: '2024-06-15',
        CashEarningsDistribution: 3.5,
        StockEarningsDistribution: 1.0,
      },
    ]
    const result = parseDividendRecords('2330', raw)
    expect(result).toHaveLength(1)
    expect(result[0]).toEqual({ symbol: '2330', exDate: '2024-06-15', cashDividend: 3.5, stockDividend: 1.0 })
  })

  test('uses StockExDividendTradingDate when cash date is empty', () => {
    const raw = [
      {
        CashExDividendTradingDate: '',
        StockExDividendTradingDate: '2024-09-20',
        CashEarningsDistribution: 0,
        StockEarningsDistribution: 2.0,
      },
    ]
    const result = parseDividendRecords('2330', raw)
    expect(result).toHaveLength(1)
    expect(result[0].exDate).toBe('2024-09-20')
    expect(result[0].stockDividend).toBe(2.0)
  })
})
